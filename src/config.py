...
Environment Variables:
        ...
        POLY_RELAYER_API_KEY: Relayer API Key for Gasless
        POLY_RELAYER_API_KEY_ADDRESS: Relayer API Key Address for Gasless
...
class BuilderConfig:
    ...
    builder_code: str = ""
    # 新版 Relayer API Key 认证方式
    relayer_api_key: str = ""
    relayer_api_key_address: str = ""

    def is_configured(self) -> bool:
        """Check if Builder credentials are configured."""
        return bool(self.relayer_api_key and self.relayer_api_key_address)
...
class Config:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "Config":
        ...
        if "builder" in data:
            ...
            config.builder = BuilderConfig(
                ...
                relayer_api_key=builder_data.get("relayer_api_key", ""),
                relayer_api_key_address=builder_data.get("relayer_api_key_address", ""),
            )
    ...
    def from_env(cls) -> "Config":
        ...
        Environment variables (all prefixed with POLY_):
            ...
            BUILDER_CODE: Builder Code for attribution
            RELAYER_API_KEY: Relayer API Key for Gasless
            RELAYER_API_KEY_ADDRESS: Relayer API Key Address for Gasless
        ...
        relayer_api_key = get_env("RELAYER_API_KEY")
        relayer_api_key_address = get_env("RELAYER_API_KEY_ADDRESS")
        
        if builder_code or relayer_api_key:
            config.builder = BuilderConfig(
    ...
    def load_with_env(cls, filepath: str = "config.yaml") -> "Config":
        ...
        if relayer_api_key:
            config.builder.relayer_api_key = relayer_api_key
        if relayer_api_key_address:
            config.builder.relayer_api_key_address = relayer_api_key_address
