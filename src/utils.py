...
def create_bot_from_env() -> TradingBot:
    ...
    Optional environment variables:
        ...
        POLY_RELAYER_API_KEY_ADDRESS: Relayer API key address
    ...
    if not private_key:
        raise ValueError(
            ...
            "Set it with: export PRIVATE_KEY=your_key"
        )

    safe_address = get_env("PROXY_WALLET")
    if not safe_address:
        raise ValueError(
            "PROXY_WALLET environment variable is required. "
            "Set it with: export PROXY_WALLET=0x..."
        )
