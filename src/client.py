...
class ClobClient(ApiClient):
    ...
    def __init__(
        ...
        self.signer_address: Optional[str] = None  # EOA address for L2 auth

    def set_signer_address(self, address: str) -> None:
        """Set the signer's EOA address for L2 authentication."""
        self.signer_address = address.lower()
    ...
    def _build_headers(
        ...
        if self.api_creds and self.api_creds.is_valid():
            ...
            try:
                # 使用普通 base64 解码（与官方 SDK 一致）
                base64_secret = base64.b64decode(self.api_creds.secret)
                h = hmac.new(base64_secret, message.encode("utf-8"), hashlib.sha256)
                # 输出使用 URL safe base64（与官方 SDK 一致）
                sig_base64 = base64.b64encode(h.digest()).decode("utf-8")
                signature = sig_base64.replace("+", "-").replace("/", "_")
            except Exception:
                # Fallback
                signature = hmac.new(
        ...
        if headers:
            ...
            poly_addr = headers.get("POLY_ADDRESS", "N/A")
            logger.info(f"Auth headers: POLY_ADDRESS={poly_addr}, signer_address={self.signer_address}, funder={self.funder}")
    ...
    def post_order(
        ...
        # Remove nonce as V2 order API does not accept it anymore
        order_data = signed_order.get("order", signed_order)
        if isinstance(order_data, dict) and "nonce" in order_data:
            order_data = dict(order_data)
            del order_data["nonce"]

        # Build request body
        body = {
            "order": order_data,
            "owner": self.api_creds.api_key if self.api_creds else "",  # 使用 API Key！
            "orderType": order_type,
        }
        
        # Add signature and signer
        if "signature" in signed_order:
            body["signature"] = signed_order["signature"]
        if "signer" in signed_order:
            body["signer"] = signed_order["signer"]  # 添加这行！
...
class RelayerClient(ApiClient):
    ...
    def _build_headers(
        ...
        """Build Relayer authentication headers."""
        if not self.builder_creds or not self.builder_creds.is_configured():
            raise AuthenticationError("Relayer credentials required for relayer")

        if self.builder_creds.relayer_api_key and self.builder_creds.relayer_api_key_address:
            return {
                "RELAYER_API_KEY": self.builder_creds.relayer_api_key,
                "RELAYER_API_KEY_ADDRESS": self.builder_creds.relayer_api_key_address,
            }
        
        raise AuthenticationError("Relayer credentials are not fully configured")
    ...
    def execute(self, transactions: List[Dict[str, Any]]) -> Dict[str, Any]:
        """
        Execute a batch of transactions via the Relayer.

        Args:
            transactions: List of transaction objects {to, value, data, operation}
    ...
    def approve_usdc(
        ...
        Args:
            safe_address: Safe address (unused in V2 execute, but kept for compatibility)
        ...
        encoded_args = encode(['address', 'uint256'], [spender, int(amount)])
        data = "0x" + selector.hex() + encoded_args.hex()
    ...
    def approve_token(
        ...
        """
        Approve an ERC-1155 token (Conditional Tokens).
        
        Note: For ERC1155, we usually use setApprovalForAll.
        The `amount` parameter is ignored for setApprovalForAll (it's boolean).
        We assume `amount > 0` means `approved=True`.

        Args:
            safe_address: Safe address
            token_id: Token ID (unused for setApprovalForAll)
            spender: Spender address
            amount: Amount (ignored, treated as boolean)
        ...
        is_approved = int(amount) > 0
        encoded_args = encode(['address', 'bool'], [spender, is_approved])
        data = "0x" + selector.hex() + encoded_args.hex()
