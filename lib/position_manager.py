...
class Position:
    ...
    # TP/SL config (set by PositionManager, representing percentages)
    take_profit_pct: float = 0.50
    stop_loss_pct: float = 0.50

    # State management
    status: str = "open"  # "pending_open", "open", "pending_close", "closed"
    hedge_order_id: Optional[str] = None
    hedge_order_time: float = 0.0
    locked_pnl: float = 0.0

    @property
    def take_profit_price(self) -> float:
        """Target price for take profit (dynamic)."""
        # Dynamic TP: entry_price * (1 + take_profit_pct)
        price = self.entry_price * (1.0 + self.take_profit_pct)
        # Cap at 0.99 (max possible token value in Polymarket)
        return min(0.99, max(0.01, price))

    @property
    def stop_loss_price(self) -> float:
        """Target price for stop loss (dynamic)."""
        # Dynamic SL: entry_price * (1 - stop_loss_pct)
        price = self.entry_price * (1.0 - self.stop_loss_pct)
        return min(0.99, max(0.01, price))
...
class PositionManager:
    ...
    take_profit: float = 0.50  # 50% dynamic TP
    stop_loss: float = 0.50    # 50% dynamic SL
    ...
    def open_position(
        ...
        Args:
            ...
            status: Initial status, default is "open"
    ...
    def confirm_open_position(self, order_id: str, actual_price: float, actual_size: float) -> Optional[Position]:
        """Confirm a pending position based on order ID."""
        for pos in self._positions.values():
            if pos.order_id == order_id and pos.status == "pending_open":
                pos.status = "open"
                if actual_price > 0:
    ...
    def remove_position(self, position_id: str) -> Optional[Position]:
        """Remove a position without recording stats (for failed pending positions)."""
        if position_id in self._positions:
            pos = self._positions.pop(position_id)
            if pos.side in self._positions_by_side and self._positions_by_side[pos.side] == position_id:
                del self._positions_by_side[pos.side]
            return pos
        return None

    def get_position_by_order_id(self, order_id: str) -> Optional[Position]:
        """Get position by open or close order ID."""
        for pos in self._positions.values():
            if pos.order_id == order_id or pos.hedge_order_id == order_id:
                return pos
        return None
    ...
    def get_unrealized_pnl(self, prices: Dict[str, float]) -> float:
        ...
        total = 0.0
        for position in self._positions.values():
            if position.status in ("closing", "pending_close"):
                total += position.locked_pnl
